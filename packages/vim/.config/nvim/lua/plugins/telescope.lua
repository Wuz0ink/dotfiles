return {
	{
		...
		config = function()
            ...
            keymap.set('n', '<leader>f', function() builtin.live_grep({ additional_args = function() return { "--hidden" } end }) end, { desc = 'Telescope live grep (inklusive dolda filer)' })
            keymap.set('n', '<leader>t', function() builtin.find_files({ hidden = true }) end, { desc = 'Telescope find hidden files' })
		end,
	}
}
